function [A, b] = jacobian_rhs_simple(params_phys, vars_sol, vars_num,rhs_version)
    ...
    if nargin <= 3
        %rhs_version = "pendant_drop";
        rhs_version = "puff_sigmoid";
        %rhs_version = "puff_exp";
    end

    switch lower(rhs_version)
        case 'puff_sigmoid'
            [A,b] = puff_sigmoid(params_phys, vars_sol, vars_num);
        case 'puff_exp'
            [A,b] = puff_exp(params_phys, vars_sol, vars_num);
        case 'pendant_drop'
            [A,b] = pendant_drop(params_phys, vars_sol, vars_num);
        otherwise
            error("Kwarg `rhs_version` of function `jacobian_rhs_simple` with value `$rhs_version` is not defined or found. \n%s", ...
                "Try passing a valid name `puff_sigmoid`, `puff_exp` or `pendant_drop` (not defining the input, defaults to `pendant_drop`)")
    end
    
end

function [A, b] = puff_sigmoid(params_phys, vars_sol, vars_num)
    % JACOBIAN_RHS_SIMPLE Computes the Jacobian matrix and RHS vector for a 
    % simple shape problem.
    %
    % INPUTS:
    ...
    a = params_phys.puff_sigmoid_a;
    b = params_phys.puff_sigmoid_b;
    pmax = params_phys.puff_sigmoid_pmax;
    pmin = params_phys.puff_sigmoid_pmin;
...
function [A, b] = puff_exp(params_phys, vars_sol, vars_num)
    % JACOBIAN_RHS_SIMPLE Computes the Jacobian matrix and RHS vector for a 
    % simple shape problem.
    %
    % INPUTS:
    ...
    a = params_phys.puff_exp_a;
    b = params_phys.puff_exp_b;
...
function [A, b] = pendant_drop(params_phys, vars_sol, vars_num)
    % JACOBIAN_RHS_SIMPLE Computes the Jacobian matrix and RHS vector for a 
    % simple shape problem.
    %
    % INPUTS:
